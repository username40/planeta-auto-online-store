<template>
    <div id="app">
        ...
        <div class="container mt30px">
            <h1 class="h1">Аккумуляторы в Екатерибурге</h1>
        </div>
                        ...
                        <div class="page-content__view-method-params">
                            ...
                            <div class="right-side">
                                ...
                                <div class="change-view-container">
                                    ...
                                    <label for="change-view_grid" class="grey-border change-view_left grey-border"><span class="change-view__icon change-view__icon_grid"></span></label>
                                    ...
                                    <label for="change-view_list" class="grey-border change-view_right grey-border"><span class="change-view__icon change-view__icon_list"></span></label>
                                </div>
                            </div>
                        </div>
                        <div class="goods-grid mt30px" v-show="showMethod === 'grid'">
                            <div class="" v-for="product in products" :key="product.id">
                                <app-good-item-grid-small :product="product"/>
                            </div>
                        </div>
                        <div class="goods-list mt30px" v-show="showMethod === 'list'">
                            <div v-for="product in products" :key="product.id">
                                <app-good-item-list :product="product"/>
                            </div>
                        </div>
                        <app-catalog-list-banner class="mt20px"/>
                        <p class="Tag-16-60 mt60px text-center">Просмотрено {{ products.length }} товаров из {{ total }}</p>
                        <button @click.prevent="showMoreProducts" class="mt30px h3 show-more-btn grey-border">Показать еще</button>
                    </div>
...
<script>
    ...
    import AsideBanner from '~/components/global/banners/AsideBanner'
    import WaitAsideBanner from '~/components/global/banners/WaitAsideBanner'
    import CatalogListBanner from '~/components/global/banners/CatalogListBanner'
    ...
    export default {
        ...
        components: {
            ...
            appRange: Range,
            appAsideBanner: AsideBanner,
            appWaitAsideBanner: WaitAsideBanner,
            appCatalogListBanner: CatalogListBanner,
        ...
        data() {
            return {
                ...
                waitFeaturesBannersData: [
                    {
                        "id": 1,
                        "image": "/images/banners/banner-image1.jpg",
                        "title": "Больше удобных фильтров в каталоге",
                        "description": "Мы работаем над улучшением магазина и скоро добавим больше фильтров для удобного поиска товаров в каталоге."
                    },
                    {
                        "id": 2,
                        "image": "/images/banners/banner-image2.jpg",
                        "title": "Больше удобных фильтров в каталоге",
                        "description": "Мы расширяем функционал магазина и скоро добавим больше фильтров для удобного поиска товаров в каталоге."
                    }
                ]
            }
        },
        methods: {
            showMoreProducts() {
                // const newProducts =  this.$axios.get(this.$store.state.products.nextPage)
                const newProducts =  this.$axios.get("https://lapi.planetavto.ru/api/v1/products?page=2")
                    // .then((newProducts) => this.$store.commit('products/setNewProducts', newProducts.data))
                    // .then((newProducts) => console.log(newProducts.data.data))
                    .then((newProducts) => {
                        this.$store.commit('products/setNextPage', newProducts.data.next_page_url)
                        this.$store.commit('products/setNewProducts', newProducts.data.data)
            })
            }
        },
        computed: {
